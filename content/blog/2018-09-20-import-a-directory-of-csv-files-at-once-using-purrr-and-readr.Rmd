...
To make the example more concrete, we'll use a dataset provided by the Government of Ireland shoing the number of e-referrals per hospital department.
...
But knowing that each list element will be a `tibble` and that each data set has the same columns, we can use [purrr]{.pkg}'s typed functions to return a single dataframe containing each of the imported CSV files using `purrr::map_dfr()`.
...
To fix this, the easiest solution is to import as a character and use [[lubridate]]{.pkg} to parse the datetimes, as [readr]{.pkg}'s `col_date()` format only allows for one format.
...
To do this, we set the `.id` parameter of `map_dfr()` to the name of the indicator variable, and the source filename will be included in the final data set.
