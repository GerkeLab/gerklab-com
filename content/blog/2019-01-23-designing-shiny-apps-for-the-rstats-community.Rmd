...
ga_data$screen_sizes <- google_analytics(
  rsconf_viewId,
  date_range = c("2019-01-14", "2019-01-23"), 
...
*note: probably want to scale by percentage of traffic*
...
ga_data$browser_os %>%
  ...
  imap(~ {
    mutate(.x, 
           ...
           text_placement = ifelse(sessions < 30, sessions, 0),
           text_hjust = ifelse(sessions < 30, -0.1, 1.05)
    ) %>% 
    ggplot() +
      ...
      scale_y_continuous(expand = expand_scale(c(0.1, 0), 0)) +
      # guides(fill = guide_legend(reverse = TRUE)) +
      guides(fill = FALSE, color = FALSE) +
      labs(y = "Sessions", x = NULL) +
      ggtitle(.y) +
...
ga_data$screen_sizes %>% 
  filter(operatingSystem != "Chrome OS") %>% 
